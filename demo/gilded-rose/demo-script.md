...
DAR (Deep Adversarial Reviews) — iterative adversarial review using Claude self-review and OpenAI GPT-5.6 cross-model verification. Finds flaws before they become bugs. Not a fixer — a reviewer.

## Demo Pattern

`/darc:1` (one round) → review findings → human applies fixes → `/darc:1` again.

DAR reviews. The human decides what to implement. This is by design.

## Setup

```bash
cd demo/gilded-rose
cp original.py gilded_rose.py
```

## Round 1: DARc reviews the original

```bash
/darc:1 gilded_rose.py
```
