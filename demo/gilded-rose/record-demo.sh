...
echo -n "> "
sleep 0.25
echo "cat original.py"
sleep 0.2
head -16 "$SCRIPT_DIR/original.py"
echo "    ..."
sleep 1

echo ""
type_cmd "/darc 1 original.py"
sleep 0.4
echo ""
...
while IFS= read -r line; do
  if [ "$IN_SELF" = true ] && echo "$line" | grep -q "Calling GPT-5.6"; then
    IN_SELF=false
    # Display the "Calling GPT-5.6..." line then break to do live call
...
if [ -x "$ENGINE" ]; then
  ...
  echo ""
  TMP=$(mktemp)
  bash "$ENGINE" "$SCRIPT_DIR/original.py" > "$TMP" 2>/dev/null
  echo ""
  # Display engine output at reading pace
...
# ── Synthesis: replay from capture (after GPT-5.6 section) ──
IN_SYNTH=false
while IFS= read -r line; do
  ...
  if echo "$line" | grep -qE "^═══"; then
    sleep 0.45
  elif echo "$line" | grep -qE "^(Both|Fable|GPT-5.6|Cross-model)"; then
    sleep 0.3
  elif [ -z "$line" ]; then
    sleep 0.12
  else
