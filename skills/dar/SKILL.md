...
- **`/DARc:config`**: Show or change adversarial model configuration. `/DARc:config` displays current provider and model. `/DARc:config provider openai|codex|claude` sets the provider. `/DARc:config model <id>` sets the model. API keys are never requested — DARc tells you which env var to set.
## Installation

1. Download `DARc.zip` from the latest release and unzip into `~/.claude/skills/DARc/`
